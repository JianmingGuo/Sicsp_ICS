...
public class Launcher {
    ...
    public static void main(String[] args) throws IOException {
        ...
        GeoIp.Initialize(filePath+"/data/cidr_to_geo_id.csv", filePath+"/data/geo_id_to_name.csv");

//        try {
//            String filePath = JarToolUtil.getJarDir();
//            BufferedReader reader = new BufferedReader(new FileReader(filePath+"/data/cidr_to_geo_id.csv"));
...
//        }
        ...
        try {
            Manufacturer.parseSourceFile(Paths.get(Configuration.getPreferenceString(Configuration.Fields.PATH_MANUFACTURER_DB))); //failed
        } catch(Exception ex) {
        ...
        if(allowPcap) {
            try {
                ...
                System.load(filePath+"/jnetpcap.dll");
                Pcap.libVersion();
        ...
        String wiresharkPath = Configuration.getPreferenceString(Configuration.Fields.WIRESHARK_EXEC);   //failed
        ...
        final Path systemFingerprintPath = Paths.get(filePath+"\\" + FingerPrintGui.getSystemFingerprintDir());
//        System.out.println(Files.list(systemFingerprintPath).toString());
        ...
        try {
            Files.list(systemFingerprintPath).forEach(path -> {    //failed
                try {
                    fpDoc.load(path);
                } catch (final JAXBException je) {
                    Logger.log(Launcher.class, Severity.Warning, "Unable to load Fingerprint at " + path);
        ...
        NetworkGraph.tab = args[1];
        MysqlDemo.tab = args[1];
    ...
    public static synchronized void LoadPlugins() {
        ...
        pluginsLoaded = true;
        String filePath = JarToolUtil.getJarDir()+"/plugins";
        try {
            for (Path pathPlugin : Files.newDirectoryStream(Paths.get(filePath))) {   //failed
