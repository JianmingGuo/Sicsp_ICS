...
public class Cell<TNode extends INode<TNode>> extends Pane implements CellLayer.ICanHazContextMenu, IDraggable {
    ...
    protected static HashMap<String,ArrayList<String>> ip_cast;
...
//            String mac = node.GetMac();
            if (!flag)
            {m.nickname();
                ip_cast = m.IP_NAME_OTHER();
                flag = true;}
            String ip = node.titleProperty().getValue();
            for (String in : ip_cast.keySet()) {
                if (ip_cast.get(in).get(0).equals(ip)){
                    name = ip_cast.get(in).get(1);
                    break;
                }
                else{if(ip_cast.get(in).get(2).contains(ip)){
                    name = ip_cast.get(in).get(1);
                    break;}
                }
            }
        ...
        if(name != null) {
            node.titleProperty().setValue(name);
        }
